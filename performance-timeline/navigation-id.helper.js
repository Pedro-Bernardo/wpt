...
let testMarkMeasure = (expectedNavigationId, markName, MeasureName) => {
  ...
  while (window.performance.now() < curr + 100);
  window.performance.mark(markName2);
  window.performance.measure(measureName, markName1, markName2);
  return window.performance.getEntriesByName(markName2).concat(window.performance.getEntriesByName(measureName)).map(e => e.navigationId);
}
